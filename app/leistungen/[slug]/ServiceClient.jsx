...
export default function ServiceClient({ service }) {
  ...
  type="button"
  onClick={() => {
    // افتح المودال حتى لو ما اخترنا باكج بعد
    setActivePackage({ slug: "standard", title: "Allgemeine Anfrage" });
    setModalOpen(true);
  }}
...
>
  Unverbindlich anfragen
      ...
      <section className="mx-auto max-w-6xl px-6 pb-6">
        ...
        <p className="mt-2 text-white/70">Wählen Sie das passende Paket – alle Preise sind Startpreise und werden an Ihr Projekt angepasst.</p>
              ...
              <div key={d.slug} className="rounded-2xl border border-white/10 p-6 flex flex-col transition-all">
                ...
                {examples.length > 0 && (
                  <div id={`pkg-${d.slug}`} className="mt-5">
                    <p className="mb-2 text-sm text-white/70">{isOpen ? "Beispiele aus dem Web:" : "Beispiel aus dem Web:"}</p>
                ...
                <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    aria-label={`Anfrage starten: ${d.title}`}
                  >
                    Starten
                  </button>
                </div>
      ...
      {s.gallery?.length > 0 && (
        <section className="mx-auto max-w-6xl px-6 py-10">
          ...
          <p className="mt-2 text-white/70">Ein Auszug unserer Arbeiten – Logos, Social-Vorlagen, Print/Packaging & mehr.</p>

          {(() => {
            const [activeType, setActiveType] = useState("Alle");
            const types = useMemo(() => ["Alle", ...Array.from(new Set(s.gallery.map((g) => g.type)))], [s.gallery]);
            const items = useMemo(() => (activeType === "Alle" ? s.gallery : s.gallery.filter((g) => g.type === activeType)), [activeType, s.gallery]);
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {types.map((t) => (
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-sm border ${activeType === t ? "bg-white/20 border-white/30" : "bg-white/5 border-white/10 hover:bg-white/10"}`}
                    >
                      {t}
                    </button>
                  ))}
